...
class pengadaanTopingModel extends Model
{
    ...
    protected $allowedFields = ['id_pengadaan_toping', 'id_user', 'ket_pengadan_toping','created_at', 'updated_at'];

    public function getPengadaanToping($id = false)
    {
        if ($id === false) {
            return $this
            ->select('pengadaan_toping.id_pengadaan_toping, pengadaan_toping.id_user, pengadaan_toping.created_at, pengadaan_toping.updated_at, users.nama_user')
